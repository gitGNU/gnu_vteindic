			 ...
			 gboolean insert, gboolean invalidate_now)
{
	...
	if (G_UNLIKELY (VTE_ISO2022_HAS_ENCODED_WIDTH(c))) {
		...
		if(c >= start_devanagari_unicode && c <= end_devanagari_unicode ){
			/* If the character is Devanagari diacretic 
			 * i.e., dependent character then to seperate 
			 * dependent characters with non-zero width 
			 * _vte_iso2022_unichar_width function returns
			 * value 4 instead of 1. So here we reset the 
			 * original colummns value, use complex_columns 
			 * for further processing.
			 * temp_columns_val is 4 #DEFINED in 
			 * indic_support.h  */

			columns = _vte_iso2022_unichar_width(terminal->pvt->iso2022, c);
			complex_columns = columns;

			/* Exceptional case handling  devanagari specific */
			check_for_exceptional_cases_devanagari(stored_char, c);	
			
			/* stores the non-zero width character */
			if (columns != 0)
				stored_char = c;
			if (columns == temp_column_val)
				columns = original_column_val;
			columns *= (devanagari_locale[c - start_devanagari_unicode].full_width 
				 + font_diff);
		}
		else {
			columns = _vte_iso2022_unichar_width(terminal->pvt->iso2022, c);
			complex_columns = columns;
			/* Reset the columns value to original which is 1. */
			if (columns == temp_column_val)
				columns = original_column_val;
			/* All calculations for allocating the rendering 
			 * space for charcter are done with assumed font 
			 * size 9. To generlize this fon_diff is calculated
			 * that corrects the rendering space required for 
			 * character. */	
			columns*=(assumed_font_scale + font_diff);
		}
	}
	
	/* If we're autowrapping here, do it. */
	if (col > 0 )
		col = col + screen->cursor_current.col;	
	else 	
	...
	if (G_UNLIKELY (columns && col + columns > (terminal->column_count))) {
		if (terminal->pvt->flags.am) {
			...
			/*
			* Autowrap the line if end of row is detected.
			* We have changed this. Previously the code here 
			* was to stay at the rightmost column
			*/
			col = screen->cursor_current.col = 0;
			/* Mark this line as soft-wrapped. */
                        row = _vte_terminal_ensure_row (terminal);
	...
	/* This will move the cursor to next position 
	 * if don't want to put non-zero width appending character */
	if(complex_columns != temp_column_val && halant_flag == state_three && columns != 0) {
		if(col != 0) {
				/* move screen cursor to next column */
				screen->cursor_current.col = col + no_of_columns_mov_cursor; 
		}	

		/* set col value to current position of screen cursor */
		col = screen->cursor_current.col;     
		halant_flag = 0;
		no_of_columns_mov_cursor = 0;
	}
       	/* Major issue discovered while testing
	 * character with halant should be combined
	 * only with the Devanagari character and 
	 * not with the other */
	if(halant_flag == state_two ){
		if(c >= 0xa8fb || c <= 0x0900)
			halant_flag = 0;
	}	

	if (G_UNLIKELY (columns == 0 || complex_columns == temp_column_val 
		|| halant_flag == state_two)) {

		/* It's a combining mark */
		long row_num;
		VteCell *cell;

		/* In case of halant we look behind for two characters. 
		 * Halant is zero width character so will get appended 
		 * to the base charcter, but the next character should 
		 * be appended to current base character. 
		 * That's why this halant flag is used to maintain the 
		 * state information. */

		if(halant_flag == state_three) {
			/* Reset the halant_flag to 0 as we have appended 
		 	 * characters using halant_flag i.e. end of state */

			halant_flag = 0;
			if(columns == 0 && c != devanagari_halant){
				zero_width = state_one;
				prev_num_columns = no_of_columns_mov_cursor;
				no_of_columns_mov_cursor = 0;
			}
			if(complex_columns == temp_column_val){
				prev_num_columns = no_of_columns_mov_cursor;
				no_of_columns_mov_cursor = 0;
			}
		}
		...
		if(complex_columns == temp_column_val) {
			/* We have to put the non-zero width character 
			 * so set the flag to 1 */
			iscomplex_flag = state_one; 
			no_halant = state_one;
		}

		/*if the current character is halant then set the halant flag to 1 */

		if(c == devanagari_halant)
			halant_flag = state_one;
		
		/* Rest part is same as previous one i.e., appending 
		 * the characters to previous character */
		...
		col--;
		cell = _vte_row_data_get_writable (row, col);
		if(halant_flag == state_one){
			if(cell->c >= start_devanagari_unicode 
				&& cell->c <= end_devanagari_unicode){
		     		diff = (devanagari_locale[cell->c
				     - start_devanagari_unicode].full_width
				     - devanagari_locale[cell->c
				     - start_devanagari_unicode].halant_width 
				     + font_diff);
			}
			else{
				diff = (devanagari_locale[stored_char
				     - start_devanagari_unicode].full_width
		...
		if(halant_flag == state_two){
			/* Devanagari Language specifix exceptions 
	 		 * handling */
			if(exceptional_devanagari_case == 1 ){
				no_of_columns_mov_cursor = 0;
				exceptional_devanagari_case = 2;
			}	
			else {	
				no_of_columns_mov_cursor = (no_of_columns_mov_cursor
					                 + devanagari_locale[c
							 - start_devanagari_unicode].full_width
					                 - diff + font_diff);
			 }		 
	...
	/* If the character is non-zero width and appended 
	   to base character the move cursor to next vte cell */

	if(zero_width == state_one) {
			screen->cursor_current.col = col + prev_num_columns;
			zero_width = 0;
			prev_num_columns = 0;
	}
	...
	/*
		Characters containing the halant has been appended 
		and done with that, have to check for next 
		character wheather it is non-zero width or not so 
		set halant flag to 3
	*/
	if(halant_flag == state_two)
		halant_flag = state_three;
	
	/*
		Halant character is apended and waiting for next 
		character which needs to be appended to base 
		character so set Halant Flag to 2
	*/
	...
	if (terminal->pvt->draw != NULL) {
		...
		if (!terminal->pvt->has_fonts) {
			vte_terminal_set_font_full_internal(terminal,
                                                            ...
                                                            terminal->pvt->fontantialias);
			font_diff=terminal->char_width-9;
		}
		if (terminal->pvt->fontdirty) {
			...
			vte_terminal_apply_metrics(terminal,
						   9+font_diff, height, ascent, height - ascent);
		}
	}
}
...
vte_terminal_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
	...
	height = (allocation->height - (terminal->pvt->inner_border.top + terminal->pvt->inner_border.bottom)) /
		 terminal->char_height;
	//printf("%d %d %d %d\n",width,height,allocation->width,terminal->char_width);
	width = MAX(width, 1);
	height = MAX(height, 1);
